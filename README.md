...
is handled on the **big domain** (`flowCaseBig`/`dispersionCaseBig`). For the flow it can be a
porous momentum sink or a snapped roughness wall (two switchable models). For the dispersion the
canopy has **two sink implementations**, either selectable per run: a total sink
(`scalarFixedValueConstraint`, T=0, complete uptake) and a finite first-order dry deposition
(`scalarSemiImplicitSource`, -λT) - see `dispersionCaseBig/constant/fvOptions`.

> **Status (updated).** The pipeline below is complete and has run end-to-end on the cluster:
> all four scenarios (reference/S1/S2/S3), the 10 representative-hour POD-snapshot run, receptor
...
**Atmospheric boundary-layer inlet (the Rotterdam case example approach).** The domain
...
**Resolved:** the single-hour driver below still works, and on top of it a **POD-snapshot
workflow** is built (`cases/workflow/Snakefile.podrun` + `run_podrun.sh`), submitting to SLURM via
Snakemake's **cluster-generic** executor; it produces the decomposed snapshots and computes the
surface + volume receptor tables and the report inputs - see `cases/workflow/README_podrun.md`
and §11.
...
referenceCase/
  ...
  cases/
    ...
    tools/
      ...
      collect_receptors.py    surface receptor postProcessing -> receptors.csv (receptor collector)
      select_hours.py clean_surface.py set_vegetation_model.py  (POD + setup helpers)
    ...
    flowCaseBig/  dispersionCaseBig/   BIG 25 km city4CFD domain (switchable porous/wall flow canopy; total-sink or deposition dispersion sink)
    postpro/                  ParaView figures: pv_dispersion_figures.py, pv_ground_slice.py, pv_mesh_inspect.py
    workflow/                 Snakemake POD-snapshot workflow -> snapshots + receptor tables + report
    round/                    uploaded Rotterdam case example (reference ABL template)
...
validated · canopy modelling on the big domain (porous/wall flow models; total-sink and deposition dispersion sinks) · 10 representative-hour POD-snapshot run · **volume-average receptor metric** (breathing air) alongside the
...
run scripts (no serial full-mesh decompose/reconstruct).
